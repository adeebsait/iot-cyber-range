...
class ComprehensivePerformanceEvaluator:
    ...
    def start_comprehensive_evaluation(self, evaluation_duration_hours=2):
        ...
        print(f"🔬 Starting comprehensive evaluation for {evaluation_duration_hours} hours")
        ...
        print(f"✅ Evaluation completed. Generating final comprehensive report...")
    ...
    def _consume_data(self, consumer_name, consumer, evaluation_run_id):
        ...
        try:
            ...
            print(f"Error in {consumer_name} consumer: {e}")
    ...
    def _process_ground_truth(self, event, evaluation_run_id):
        ...
        print(f"📝 Ground truth recorded: {event['event_type']} - {event.get('description', '')}")
    ...
    def calculate_comprehensive_metrics(self, evaluation_run_id):
        ...
        WHERE detection_id LIKE ?
        """
        detector_types = pd.read_sql_query(
            detector_types_query, conn, params=[f'%{evaluation_run_id}%']
        )['detector_type'].tolist()

        results = {}

        for detector_type in detector_types:
            print(f"📊 Calculating metrics for {detector_type}...")
    ...
    def _calculate_confusion_matrix(self, detector_type, evaluation_run_id, conn):
        ...
        total_normal = len(ground_truth_df[ground_truth_df['event_type'] == 'normal'])
    ...
    def _generate_performance_visualizations(self, results, evaluation_run_id):
        ...
        fig.suptitle('Comprehensive Multi-Agent System Performance Analysis', fontsize=16)
        ...
        f1_cis = [results[d]['metrics']['f1_ci'] for d in detector_names]
        f1_errors = [[f1_scores[i] - ci[0], ci[1] - f1_scores[i]] for i, ci in enumerate(f1_cis)]
        f1_errors = np.array(f1_errors).T

        axes[0, 0].bar(detector_names, f1_scores, yerr=f1_errors, capsize=5, alpha=0.7)
        axes[0, 0].set_title('F1-Score with 95% Confidence Intervals')
        ...
        latency_errors = [results[d]['latency_stats']['std'] for d in detector_names]

        axes[0, 2].bar(detector_names, latencies, yerr=latency_errors, capsize=5, alpha=0.7)
        axes[0, 2].set_title('Average Detection Latency')
        axes[0, 2].set_ylabel('Latency (seconds)')
        axes[0, 2].tick_params(axis='x', rotation=45)

        # 4. Confusion Matrix Heatmap for Multi-Agent
        if 'multi_agent' in results:
            cm = results['multi_agent']['confusion_matrix']
            cm_matrix = np.array([[cm['tp'], cm['fp']], [cm['fn'], cm['tn']]])
            sns.heatmap(cm_matrix, annot=True, fmt='d',
                        xticklabels=['Predicted Positive', 'Predicted Negative'],
                        yticklabels=['Actual Positive', 'Actual Negative'],
                        ax=axes[1, 0], cmap='Blues')
            axes[1, 0].set_title('Multi-Agent Confusion Matrix')

        # 5. False Positive Rate Comparison
        fpr_rates = [results[d]['metrics']['false_positive_rate'] for d in detector_names]

        axes[1, 1].bar(detector_names, fpr_rates, alpha=0.7, color='orange')
        axes[1, 1].set_title('False Positive Rates')
        axes[1, 1].set_ylabel('False Positive Rate')
        axes[1, 1].tick_params(axis='x', rotation=45)

        # 6. Overall Performance Radar Chart
        metrics_for_radar = ['precision', 'recall', 'f1_score', 'accuracy']
        ...
        axes[1, 2].set_thetagrids(np.degrees(angles[:-1]), metrics_for_radar)

        for detector in detector_names[:3]:  # Limit to first 3 for readability
            values = [results[detector]['metrics'][metric] for metric in metrics_for_radar]
            values += [values[0]]  # Close the polygon
            axes[1, 2].plot(angles, values, 'o-', linewidth=2, label=detector.replace('_', ' ').title())
        ...
        print(f"📊 Performance visualizations saved as comprehensive_performance_{evaluation_run_id}.png")
    ...
    def _print_evaluation_summary(self, report):
        ...
        print(f"Detectors Evaluated: {report['summary']['total_detectors_evaluated']}")
        ...
        for detector, results in report['detector_results'].items():
            ...
            print(f"  Precision: {results['metrics']['precision']:.3f} "
                  f"(CI: {results['metrics']['precision_ci'][0]:.3f}-{results['metrics']['precision_ci'][1]:.3f})")
            print(f"  Recall: {results['metrics']['recall']:.3f} "
                  f"(CI: {results['metrics']['recall_ci'][0]:.3f}-{results['metrics']['recall_ci'][1]:.3f})")
            print(f"  F1-Score: {results['metrics']['f1_score']:.3f} "
                  f"(CI: {results['metrics']['f1_ci'][0]:.3f}-{results['metrics']['f1_ci'][1]:.3f})")
            print(f"  Accuracy: {results['metrics']['accuracy']:.3f}")
            print(f"  FPR: {results['metrics']['false_positive_rate']:.3f}")
            print(f"  Avg Latency: {results['latency_stats']['mean']:.2f}s "
                  f"(±{results['latency_stats']['std']:.2f}s)")
